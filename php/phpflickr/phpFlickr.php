...
/* 			$this->parsed_response = $this->clean_text_nodes(json_decode($this->response, TRUE)); */
		...
		*******************************/
		...
		function call ($method, $arguments, $nocache = false) {
			foreach ( $arguments as $key => $value ) {
				if ( is_null($value) ) unset($arguments[$key]);
			}
			$this->request($method, $arguments, $nocache);
		...
		*/
		...
		function photos_getInfo ($photo_id, $secret = NULL, $humandates = NULL, $privacy_filter = NULL, $get_contexts = NULL) {
			...
			return $this->call('flickr.photos.getInfo', array('photo_id' => $photo_id, 'secret' => $secret, 'humandates' => $humandates, 'privacy_filter' => $privacy_filter, 'get_contexts' => $get_contexts), true);
		}
